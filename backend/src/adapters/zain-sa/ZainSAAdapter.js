/**
 * Zain SA Adapter - SLA v2.2 COMPLIANT
 * 
 * Implements SLA Digital integration for Zain Saudi Arabia operator.
 * FIXED: Correct operator code 'zain-sa' per official SLA Digital documentation
...
class ZainSAAdapter extends BaseAdapter {
  constructor(config) {
    super({
      ...config,
      operatorCode: 'zain-sa', // ✅ FIXED: Correct code per SLA Digital docs
      msisdnRegex: /^(\+966|966)?5[0-9]\d{7}$/, // Saudi mobile numbers
      endpoints: {
        // Use special Zain checkout endpoint for KSA
      ...
      businessRules: {
        createSubscription: {
          maxAmount: 30, // SAR monthly limit per docs
          minAmount: 1,
  ...
  /**
   * Create subscription with Zain SA specific logic - SLA v2.2 COMPLIANT
   */
  async createSubscription(params) {
    return this.executeWithLogging('createSubscription', params, async () => {
      ...
      const payload = {
        ...
        operator_code: 'zain-sa', // ✅ FIXED: Correct operator code
        country_code: 'SA'
      };
      ...
      if (params.pin) {
        if (!/^\d{4,6}$/.test(params.pin)) {
          throw new UnifiedError('INVALID_PIN_FORMAT', 
            'Zain SA requires 4-6 digit PIN');
        }
      ...
      if (response.data && response.data.checkout_required) {
        response.data.checkout_url = `${this.getEndpoint('checkout')}/zain-sa/${response.data.uuid}`;
      }
  ...
  async generatePIN(msisdn, campaign, chargeAmount) {
    return this.executeWithLogging('generatePIN', { msisdn, campaign, chargeAmount }, async () => {
      ...
      // Include charge amount for KSA PIN API as per documentation
      if (chargeAmount) {
  ...
  /**
   * Map Zain SA response data to unified format
  ...
  mapResponseData(response) {
    ...
    return {
      ...
      // Handle missing billing data for KSA
      ...
      operatorCode: 'zain-sa', // ✅ FIXED: Correct operator code
      countryCode: 'SA'
    };
  }
  
  /**
   * Map Zain SA errors to unified format
   */
  mapError(error) {
    const errorMappings = {
    ...
    return {
      code: 'UNMAPPED_ERROR',
      message: error.message || 'Unknown error from Zain SA',
      originalCode: errorCode,
      originalMessage: error.message,
      operatorCode: 'zain-sa' // ✅ FIXED: Correct operator code
    };
  ...
  /**
   * Map Zain SA status to unified status - CORRECTED with SDP mapping
   */
  mapStatus(operatorStatus) {
    const statusMappings = {
      // SDP-specific mappings for Zain SA 
